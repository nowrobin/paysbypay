...
import MerchantSortDropdown from '@/app/merchants/component/sortDropDown';
import MerchantStats from '@/app/merchants/component/stats';
...
function MerchantsPage() {
  ...
  return (
    <div className="flex flex-col bg-blue-100 p-2 px-6 h-screen w-[1200px] gap-4">
      <h1 className="text-4xl">가맹점 관리</h1>
      ...
      <div className="flex flex-row justify-between">
        <div className="flex flex-row gap-2 relative w-3/7">
  ...
  "
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 w-full gap-4 justify-items-stretch max-h-5/6 overflow-y-scroll scrollbar-hide">
        {filteredAndSorted?.map(merchant => (
          <Card key={merchant.mchtCode} {...merchant} />
        ))}
      </section>
    </div>
  );
